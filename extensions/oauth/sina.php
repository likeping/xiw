 ...
 * user_info
 */
function get_user_info($config) {
    $session = new session();
	$token   = $session->get('token');
	$c   = new SaeTClientV2($config['appid'] , $config['appkey'] , $token['access_token']);
	$ms  = $c->home_timeline(); // done
	$uid_get = $c->get_uid();
	$uid = $uid_get['uid'];
	$m   = $c->show_user_by_id($uid);//根据ID获取用户等基本信息
...
function get_callback_url() {
	...
	return $callback . url('member/login/callback', array('app'=>'sina'));
}
...
function oauth_callback($config) {
	...
	if ($token) {
		...
		$session->set('oauth_openid', $token['access_token']);
	}
}
