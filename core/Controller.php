...
abstract class Controller extends Fn_base {
	 ...
	 * 记录$_GET中的非法字符
	 */
	private static function check_Get($var) {
	    if (in_array($var, array('s', 'a', 'c'))) return false;
		static $cfg = null;
	 ...
	 * 记录$_POST中的非法字符
	 */
	private static function check_Post($var, $a=0) {
	    static $cfg = null;
	    ...
	    if ($cfg['SYS_ATTACK_LOG']) {
			...
			if ($body) {
				...
				foreach ($fdata as $v) {
					if (empty($v)) continue;
					$t = unserialize($v);
					//相同地址在20秒内都含有非法字符不再保存提醒
					if ($data['time'] - $t['time'] < 20 && $data['user'] == $t['user'] && $data['ip'] == $t['ip'] && $data['url'] == $t['url']) {
			...
			if ($cfg['SYS_ATTACK_MAIL'] && check::is_email($cfg['SITE_SYSMAIL'])) {
				//发送邮件至管理员
				mail::set($cfg);
				$body = '------------------------------------------------------------------------------------------<br>' .
				'SITE: ' . SITE_URL .
