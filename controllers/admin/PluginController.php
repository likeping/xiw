...
class PluginController extends Admin {
	...
	public function setAction() {
	    ...
	    if ($this->post('submit')) {
	        ...
	        $this->adminMsg(lang('success'), url('admin/plugin/set/', array('pluginid'=>$pluginid)));
	    }
	...
	public function addAction() {
	    ...
	    $this->adminMsg(lang('a-plu-6'), url('admin/plugin/index'), 3, 0, 1);
	}
	...
	public function delAction() {
	    ...
	    $this->adminMsg(lang('a-plu-11'), url('admin/plugin/index'), 3, 0, 1);
	}
	...
	public function unlinkAction() {
		...
		if (is_dir($this->dir . $dir)) {
			...
			$this->adminMsg(lang('a-plu-14'), url('admin/plugin/index'), 3, 1, 1);
		} else {
	...
	public function disableAction() {
	    ...
	    $this->adminMsg(lang('success'), url('admin/plugin/index/'), 3, 1, 1);
	}
	...
	public function ajaxviewAction() {
	    ...
	    $msg  = "<textarea id='p_" . $pluginid . "' style='font-size:12px;width:100%;height:40px;overflow:hidden;'>";
	    $msg .= "{plugin('" . $data['dir'] . "')}";
	    $msg .= "</textarea>";
	    echo $msg;
	}
	...
	public function ajaxupdateAction() {
		...
		$version = fn_geturl('http://app.finecms.net/index.php?c=api&a=version&markid=' . $data['markid']);
	...
	public function onlineAction() {
		...
		$this->view->assign('url', 'http://app.finecms.net/?v=2&admin=' . ADMIN_NAMESPACE . '&site=' . $site . '&name=' . $name . '&data=' . $data . '&version=' . CMS_VERSION);
	...
	public function downAction() {
		...
		if ($zip->extract(PCLZIP_OPT_PATH, $testzip_path, PCLZIP_OPT_REPLACE_NEWER) == 0) {
			...
			//配置文件验证
			if (filesize($testzip_path . DIRECTORY_SEPARATOR . 'config.php') == 0) $this->adminMsg(lang('a-plu-30'));
			//md5文件校验
		...
		if (empty($install)) {
			...
			if (is_dir(PLUGIN_DIR . $dir) && is_file(PLUGIN_DIR . $dir . DIRECTORY_SEPARATOR . 'config.php')) {
			    $this->adminMsg(lang('a-plu-33'), url('admin/plugin/'));
			} else {
