...
class LinkageController extends Admin {
	...
	public function listAction() {
		...
		foreach($result as $t){
			...
			$sdata[$t['id']]['option'] .= '<a href="' . url('admin/linkage/addson', array('id'=>$t['id'])) . '">' . lang('a-add') . '</a> | <a href="' . url('admin/linkage/editson', array('id'=>$t['id'])) . '">' . lang('a-edit') . '</a> | <a href="javascript:del(' . $t['id'] . ');">' . lang('a-del') . '</a>  ';
		}
    ...
    public function cacheAction($show=0) {
	    $qok  = $this->get('qok');
		if ($show == 0 && !$qok) $this->adminMsg(lang('a-lin-8'), url('admin/linkage/cache', array('qok'=>1)), 0, 1, 2);
		$data = $this->link->where('keyid=0')->select();
		$list = array();
		foreach ($data as $t) {
		    $keyid = $t['id'];
		    $sdata = $this->link->from('linkage')->where('keyid=' . $keyid)->order(array('parentid ASC', 'listorder ASC'))->select();
			$list[$keyid] = $t;
