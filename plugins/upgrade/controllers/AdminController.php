...
class AdminController extends Plugin {
    ...
    public function __construct() {
		...
		$this->file_url = 'http://down.finecms.net/';
		$this->data_url = 'http://www.finecms.net/index.php?c=sys&a=verlist';
	...
	public function indexAction() {
	    if ($this->isPostForm()) {
		    $this->adminMsg('正在为您升级，请不要关闭浏览器 ...', purl('admin/upgrade'));
		}
	...
	public function upgradeAction() {
	    ...
	    $data = $this->getFileData();
		if ($data) {
			foreach($data as $version=>$v) {
				//远程压缩包地址
				$upgradezip_url = $this->file_url . $version . '.zip';
				//保存到本地地址
				...
				if ($zip->extract(PCLZIP_OPT_PATH, $testzip_path, PCLZIP_OPT_REPLACE_NEWER) == 0) {
					...
					if (file_exists($md5_file)) {
					    if (filesize($md5_file) == 0) {
						    @unlink($upgradezip_path);
							$this->deletedir($testzip_path);
						    $this->adminMsg('升级包校验文件不正确，建议手动下载升级包！');
						}
						    ...
						    foreach ($md5s as $md5=>$file) {
							    if (file_exists($testzip_path . $file)) {
								    if (strtolower(md5(file_get_contents($testzip_path . $file))) != $md5) {
						                @unlink($upgradezip_path);
							            $this->deletedir($testzip_path);
									    $this->adminMsg('升级文件校验失败，建议手动下载升级包！');
									}
								}
							}
						} else {
	...
	private function getFileData() {
		...
		foreach ($data as $id=>$v) {
		    if (CMS_VERSION == $v['version']) $now_id = $id;
		}
		foreach ($data as $id=>$v) {
		    if ($now_id < $id) $list[$v['version']] = $v['update'];
		}
		return $list;
	}
